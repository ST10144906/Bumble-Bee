...
public class AuditController : Controller
{
    ...
    //[HttpPost]
    //public async Task<IActionResult> AuditTransactions()
    //{
    //    var userId = HttpContext.Session.GetString("FirebaseUserId") ?? "UnknownUser";
    ...
    //    try
    //    {
    //        Console.WriteLine("Querying donations collection...");
    ...
    //        if (donationsSnapshot.Documents.Count == 0)
    //        {
    //            TempData["ErrorMessage"] = "No donations found to audit.";
    //            return View("AuditDashboard");
    //        }

    //        foreach (var donationDocument in donationsSnapshot.Documents)
    //        {
    //            var donation = donationDocument.ToDictionary();
    //            string donationId = donationDocument.Id;

    //            // Validate fields
    //            if (!donation.ContainsKey("Amount") || !donation.ContainsKey("ProjectName") || !donation.ContainsKey("PaymentType"))
    //            {
    //                Console.WriteLine($"Skipping invalid donation entry: {donationId}");
    //                continue;
    //            }

    //            // Check if DonationId exists in auditLogs collection
    //            Query existingAuditQuery = _firestoreDb.Collection("auditLogs").WhereEqualTo("DonationId", donationId);
    //            QuerySnapshot existingAuditSnapshot = await existingAuditQuery.GetSnapshotAsync();

    //            if (existingAuditSnapshot.Documents.Any())
    //            {
    //                Console.WriteLine($"Donation {donationId} has already been audited. Skipping.");
    //                continue; // Skip this donation if it has already been audited
    //            }

    //            // Create an AuditLog entry for the donation
    //            var auditLog = new AuditLog
    //            {
    //                AuditLogId = Guid.NewGuid().ToString(),
    //                DonationId = donationId,
    ...
    //            // Save the AuditLog to Firestore
    //            await _firestoreDb.Collection("auditLogs").Document(auditLog.AuditLogId).SetAsync(new Dictionary<string, object>
    //        {
    //            { "AuditLogId", auditLog.AuditLogId },
    //            { "DonationId", auditLog.DonationId },
    ...
    //            Console.WriteLine($"Donation {donationId} audited successfully.");
    //        }
    //    }
    //    catch (Exception ex)
    //    {
    //        Console.WriteLine($"Error auditing donations: {ex.Message}");
    //        TempData["ErrorMessage"] = $"An error occurred while auditing donations: {ex.Message}. Please try again later.";
    //        return View("AuditDashboard");
    //    }

    //    Console.WriteLine("Auditing completed successfully.");
    //    return View("AuditResults", auditLogs);
    //}
